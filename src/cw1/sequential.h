...
bool intarray2bmp(const std::string &filename, doubleType **intarray,
                  unsigned rows, unsigned columns, doubleType min_value,
  ...
  for (unsigned row = rows; row; row--) // bottom-to-top
  {
    for (unsigned col = 0; col < columns; col++) // left-to-right
    {
      ...
      // If we haven't overrun the end of our input, convert it to a grayscale
      // value.
      // Input is clamped to the range [0, 1], where 0 --> black and 1 -->
      // white.
      //  cout << (row - 1) << " - " << col << endl;
